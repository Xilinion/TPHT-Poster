...
\newcommand{\gsec}[1]{%
  \par\vspace{0.1cm}
  \begin{center}
    {\fontsize{28}{32}\selectfont\bfseries\color{CornellRed}#1\par}
  \end{center}
  \vspace{-0.1cm}
...
\newcommand{\stat}[3]{%
  \begin{minipage}[t]{#1}
    \centering
    {\fontsize{46}{48}\selectfont\bfseries\color{CornellRed}#2}\\[0.2cm]
...
{\fontsize{31}{40}\selectfont
 A hash table can be \textcolor{CornellRed}{\textbf{smaller than the data
 it stores}} --- and still \textcolor{CornellRed}{\textbf{faster than the
...
\vspace{0.2cm}
\begin{center}
\resizebox{0.48\linewidth}{!}{\slidefonts
...
\vspace{0.2cm}
\begin{center}
\resizebox{0.68\linewidth}{!}{\slidefonts
...
\vspace{0.2cm}
\begin{center}
\resizebox{0.60\linewidth}{!}{\slidefonts
\begin{tikzpicture}[x=1cm,y=1cm,every node/.style={font=\normalsize},
    flow/.style={-{Latex[length=2.4mm,width=1.7mm]},line width=1.0pt},
    fieldbrace/.style={decorate,decoration={brace,amplitude=3.5pt,raise=2.5pt},line width=0.7pt}]
...
\vspace{0.2cm}
\begin{center}
\resizebox{0.77\linewidth}{!}{\slidefonts
\begin{tikzpicture}[x=1cm,y=1cm,every node/.style={font=\normalsize},
    flow/.style={-{Latex[length=2.4mm,width=1.7mm]},line width=1.0pt},
...
\end{center}
\vspace{0.05cm}
{\small\color{BlueGray}\textsuperscript{\dag}The one-round Feistel
permutation of Ingredient 2.\par}
\vspace{0.35cm}
\begin{itemize}
...
\vspace{0.4cm}
\psub{Making it fast: packing chains into cache lines}
One cache line per chain? \textbf{Mostly empty.} Let
\textbf{neighboring quotient groups share} one line: tight packing, but
overflow still chains.\par
\vspace{0.2cm}
\begin{center}
\resizebox{0.77\linewidth}{!}{\slidefonts
...
Keep the packing, drop the chains: a full home line migrates \textbf{one}
tuple out, leaving a tiny pointer --- overflow \keyterm{never points to
more overflow}.\par
\vspace{0.2cm}
\begin{center}
\resizebox{0.48\linewidth}{!}{\slidefonts
...
\psub{Headline numbers}
\stat{\linewidth}{105.4\%}{space efficiency: \Chained's payload exceeds
  its memory}\\[0.45cm]
\stat{\linewidth}{2.3$\times$}{the fastest baseline's YCSB throughput
  (\Flat)}\\[0.45cm]
\stat{\linewidth}{1.25}{memory accesses per operation --- overflow
  never chains}
\end{minipage}\hfill
...
\begin{tikzpicture}
  \begin{axis}[
    ybar, width=0.97\linewidth, height=7.0cm, ymin=0, ymax=545,
...
Above \keyterm{100\%}, the table stores more payload bits than it
allocates --- quotienting's positional bits are free.\par
\vspace{0.25cm}
\begin{center}
\begin{tikzpicture}
  \begin{axis}[
    ybar, width=0.97\linewidth, height=7.0cm, ymin=0, ymax=132,
...
Single-core insertion sweep. A curve \textbf{ends where the table gives
up}: baselines quit at 70--82\%; \Chained runs past \keyterm{105\%}.\par
\vspace{0.25cm}
\begin{center}
\begin{tikzpicture}
  \begin{axis}[
    width=0.99\linewidth, height=7.3cm,
